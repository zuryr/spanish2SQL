...
class SemanticEvaluator:
    ...
    def is_time(self, string_to_evaluate: str):
        ...
        return (re.match(pattern_time, string_to_evaluate) is not None) or (
            re.match(pattern_date, string_to_evaluate) is not None
        )
    ...
    def query_is_correct(self, query: Query) -> bool:
        ...
        if not query.condition.is_empty():
            ...
            if conditional_column.datatype == DataTypes.TEXT.value and (
                (conditional_operator not in valid_varchar_operators)
            ):
                return False

            if conditional_column.datatype == DataTypes.NUMBER.value and (
                (conditional_operator not in valid_numeric_operators)
                or (not self.has_numbers(conditional_value))
            ):
                return False

            if conditional_column.datatype == DataTypes.TIME.value and (
                (conditional_operator not in valid_numeric_operators)
                or (not self.is_time(conditional_value))
            ):
                return False
